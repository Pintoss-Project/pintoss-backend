...
public class OrderController {
    ...
    @GetMapping("/{userId}")
    public ApiResponse<List<OrderResponse>> getOrdersByUserId(@PathVariable @NotNull Long userId) {
        List<OrderResponse> orders = orderService.findAllByUserId(userId);
        return ApiResponse.ok(orders);
    }

    @PostMapping("/{userId}")
    public ApiResponse<Long> createOrder(@PathVariable @NotNull Long userId, @RequestBody @Valid OrderRequest requestDTO) {
